      ...
      try {
        ...
        if (hasActiveFilters) {
          ...
          const searchQuery = {
            ...
            minPriceCents: debouncedMinPrice
              ? Math.round(parseFloat(debouncedMinPrice) * 100)
              : null,
            maxPriceCents: debouncedMaxPrice
              ? Math.round(parseFloat(debouncedMaxPrice) * 100)
              : null,
            sortRule: sortRule || null,
            attributeFilters:
              attributeFiltersArray.length > 0 ? attributeFiltersArray : null,
          };
    ...
    <div className="space-y-12">
          ...
          {hasActiveFilters && (
            ...
            >
              {debouncedSearchTerm ? (
                <button
                  onClick={() => setSearchTerm("")}
                  className="px-3 py-1 rounded-full bg-stone-100 text-stone-700 text-sm inline-flex items-center gap-1"
                >
              ...
              {Object.entries(attributeFilters).flatMap(([name, values]) =>
                values.map((v) => (
                  <button
                    key={`${name}:${v}`}
                    onClick={() => handleAttributeFilterChange(name, v)}
                    className="px-3 py-1 rounded-full bg-stone-100 text-stone-700 text-sm inline-flex items-center gap-1"
                  >
                    <span>
                      {name}: {v}
      ...
      {showMobileFilters && (
        ...
        >
          ...
          <div
            ...
            className="absolute inset-y-0 right-0 w-full max-w-md bg-white shadow-xl overflow-y-auto transform transition-transform duration-300 ease-out translate-x-0"
          >
            <div className="p-6">
      ...
      {showMobileSort && (
        ...
        >
          ...
          <div
            ref={sortOverlayRef}
            className="absolute bottom-0 w-full bg-white rounded-t-2xl shadow-xl transform transition-transform duration-300 ease-out"
          >
            <div className="p-6">
      ...
      <div className="lg:hidden fixed bottom-4 inset-x-0 px-4">
        <div className="mx-auto max-w-3xl rounded-full bg-white shadow-lg border border-stone-200 px-4 py-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Shopping Cart Button */}
            <button
              onClick={toggleCart} // from useCart
              className="px-4 py-2 rounded-full border border-stone-300 text-stone-700 hover:bg-stone-100 flex items-center gap-1 cursor-pointer"
            >
              {/* Simple cart SVG, same style as elsewhere */}
              <svg
              ...
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2 9m12-9l2 9m-12 0h12"
                />
              </svg>
            ...
            <button
              ...
              className="px-4 py-2 rounded-full border border-stone-300 text-stone-700 hover:bg-stone-100"
            >
              Sort
            </button>
