...
export default function ProductCard({ product }) {
  ...
  // Derive image URL if available (support urlImage, imageUrl, images[0].url). Fallback to placeholder.
  const rawImage = product.urlImage ?? product.imageUrl ?? product.image ?? product.images?.[0]?.url;
  const imageUrl = (typeof rawImage === 'string' && rawImage.trim().length > 0) ? rawImage : null;
  // Fallback to random coffee image service for demo
  ...
  return (
    <div className="group flex flex-col overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-0.5">
      {/* Image */}
      ...
      <div className="flex flex-grow flex-col p-5">
        ...
        <p className="text-sm font-medium capitalize text-stone-500">{product.category?.toLowerCase?.() || ''}</p>
              ...
              <div className="flex flex-nowrap overflow-x-auto no-scrollbar items-center gap-2 h-8" aria-label="Select roast">
                {roasts.map((r) => (
                  <button
                    key={r}
                    type="button"
                    onClick={() => setSelectedRoast(r === selectedRoast ? null : r)}
            ...
            <div className="flex flex-nowrap overflow-x-auto no-scrollbar items-center gap-2 h-8">
              {variants.map((v, idx) => (
                <button
                  ...
                  className={`px-3 h-8 inline-flex items-center rounded-full border whitespace-nowrap leading-none ${selectedVariantIndex === idx ? 'border-amber-500 bg-amber-50 text-amber-800' : 'border-stone-300 text-stone-700 hover:bg-stone-100'} ${chipTextClass(formatPrice(v.priceCents, v.currency || firstVariant?.currency || 'EUR'))}`}
                  aria-pressed={selectedVariantIndex === idx}
                  title={v.sku || `Variant ${idx+1}`}
                >
                  {formatPrice(v.priceCents, v.currency || firstVariant?.currency || 'EUR')}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="mt-auto grid grid-cols-[1fr_auto] items-center gap-3 pt-4 min-h-[48px]">
          ...
          {requiresSelection && !selectionSatisfied ? (
            <button disabled className="rounded-md bg-stone-200 px-5 py-2.5 text-sm font-medium text-stone-600 shadow-sm cursor-not-allowed whitespace-nowrap">
              Select options
            </button>
          ) : (
            <button onClick={handleAdd} className="rounded-md bg-amber-700 px-5 py-2.5 text-sm font-medium text-white shadow-sm transition-colors hover:bg-amber-800 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 whitespace-nowrap">
              Add to Cart
            </button>
          )}
        </div>
