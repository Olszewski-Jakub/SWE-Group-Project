...
export default function ProductCard({ product }) {
  ...
  const imageUrl = (() => {
    ...
    const isAbsolute =
      /^(?:[a-z]+:)?\/\//i.test(rawImageUrl) || rawImageUrl.startsWith("data:");
  ...
  const rating =
    typeof product.rating === "number" ? product.rating : product.averageRating;
  const reviewCount =
    product.reviewCount || product.reviewsCount || product.numReviews;
  ...
  const { sizes, roasts } = useMemo(() => {
    ...
    return {
      sizes: Array.from(result.sizes),
      roasts: Array.from(result.roasts),
    };
  }, [variants]);
  ...
  const requiresSelection =
    sizes.length > 1 || roasts.length > 1 || showGenericVariantSwatches;
  const selectionSatisfied =
    (!sizes.length || selectedSize) &&
    (!roasts.length || selectedRoast) &&
    (!showGenericVariantSwatches || selectedVariantIndex !== null);
  ...
  const displayPriceText = (() => {
    if (showGenericVariantSwatches) {
      if (selectedVariantIndex !== null) {
        ...
        return formatPrice(
          v?.priceCents,
          v?.currency || firstVariant?.currency || "EUR"
        );
      }
  ...
  const handleAddToCart = async () => {
    ...
    if (!variantId) {
      console.error("Variant ID is missing for product:", product);
      return;
    }
    try {
  ...
  return (
    <Link
      ...
      className="group flex flex-col overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-0.5"
    >
      {/* Image */}
      ...
      <div className="flex flex-grow flex-col p-5">
        ...
        {(sizes.length > 0 || roasts.length > 0) && (
          <div className="mt-3 space-y-2">
            {sizes.length > 0 && (
              <div
                className="flex flex-nowrap overflow-x-auto no-scrollbar items-center gap-2 h-8"
              ...
              >
                {sizes.map((s) => (
                  <button
                    ...
                    className={`px-3 h-8 inline-flex items-center rounded-full border whitespace-nowrap leading-none ${
                      selectedSize === s
                        ? "border-amber-500 bg-amber-50 text-amber-800"
                        : "border-stone-300 text-stone-700 hover:bg-stone-100"
                    } ${chipTextClass(String(s))}`}
                ...
                {roasts.map((r) => (
                  <button
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      setSelectedRoast(r === selectedRoast ? null : r);
                    }}
                    className={`px-3 h-8 inline-flex items-center rounded-full border capitalize whitespace-nowrap leading-none ${
                      selectedRoast === r
                        ? "border-amber-500 bg-amber-50 text-amber-800"
                        : "border-stone-300 text-stone-700 hover:bg-stone-100"
                    } ${chipTextClass(String(r))}`}
                ...
                <button
                  ...
                  onClick={(e) => {
                    ...
                    setSelectedVariantIndex(
                      selectedVariantIndex === idx ? null : idx
                    );
                  }}
                  className={`px-3 h-8 inline-flex items-center rounded-full border whitespace-nowrap leading-none ${
                    selectedVariantIndex === idx
                      ? "border-amber-500 bg-amber-50 text-amber-800"
                      : "border-stone-300 text-stone-700 hover:bg-stone-100"
                  } ${chipTextClass(v.sku || `Variant ${idx + 1}`)}`}
        ...
        <div className="mt-auto grid grid-cols-[1fr_auto] items-center gap-3 pt-4 min-h-[48px]">
          <div className="min-w-0">
            <span className="text-2xl leading-tight font-bold text-stone-900 truncate">
              {displayPriceText}
            </span>
          </div>
