...
public class AdminRoleController {
    ...
    public ResponseEntity<Map<String, Object>> assign(@PathVariable("userId") String userId, @PathVariable("role") String role) {
        var res = assign.execute(userId, role);
        return ResponseEntity.ok(Map.of("userId", res.userId(), "roles", res.roles()));
    }

    @DeleteMapping(path = "/{userId}/roles/{role}")
    public ResponseEntity<Map<String, Object>> revoke(@PathVariable("userId") String userId, @PathVariable("role") String role) {
