...
public class ShoppingCart {
    ...
    public CartItems items() { return items; }

    /** Adds an item; only allowed if the cart is active. */
    public void addItem(Variant variant, int quantity) { modify(() -> items.add(variant, quantity)); }

    /** Removes an item. */
    public void removeItem(Variant variant) { modify(() -> items.remove(variant)); }

    /** Updates an item's quantity, adding or removing if necessary. */
    public void updateItemQuantity(Variant variant, int newQuantity) { modify(() -> items.updateQuantity(variant, newQuantity)); }

    /** Clears all items from the cart. */
    public void clear() { modify(items::clear); }

    /** Marks the cart as checked out. */
    public void checkout() { status = CartStatus.CHECKED_OUT; updatedAt = Instant.now(); }

    /** Marks the cart as abandoned. */
    public void abandon() { status = CartStatus.ABANDONED; updatedAt = Instant.now(); }

    /** @return total number of individual items in the cart */
    public int getTotalItems() {
        return items.asList().stream().mapToInt(CartItem::quantity).sum();
    }
