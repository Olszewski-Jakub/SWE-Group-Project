...
/**
 * Codec for {@link ie.universityofgalway.groupnine.domain.email.jobs.OrderRefundedEmailJob}
 * model payloads. Converts between the job model and JSON, leaving envelope handling to the
 * composite serializer.
 */
@Component
public class OrderRefundedEmailJobCodec implements EmailJobCodec<OrderRefundedEmailJob> {
    private final ObjectMapper mapper = new ObjectMapper();

    /** @return the email type this codec supports */
    @Override public EmailType type() { return EmailType.ORDER_REFUNDED; }

    /** Serializes the job model map to a JSON node. */
    @Override public JsonNode writeModel(OrderRefundedEmailJob job) {
    ...
    @Override public OrderRefundedEmailJob readModel(JsonNode modelNode, Envelope env) {
        ...
        OrderRefundedEmailJob.OptionalStep b = OrderRefundedEmailJob.builder().to(to).id(id).locale(locale).priority(pr).createdAt(createdAt);
